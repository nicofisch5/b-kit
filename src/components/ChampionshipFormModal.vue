  ...
  <div class="modal-backdrop" @click.self="$emit('cancel')">
    <div class="modal">
      <div class="modal-header">
        <h2>{{ initial ? 'Edit Championship' : 'New Championship' }}</h2>
        <button class="close-btn" @click="$emit('cancel')">✕</button>
      </div>
      <div class="modal-body">
        <div class="field">
          <label>Name</label>
          <input v-model="name" type="text" placeholder="e.g. Regional U15" maxlength="100" @keyup.enter="save" autofocus />
        </div>
...
async function save() {
  ...
  try {
    const result = props.initial
      ? await championshipService.update(props.initial.id, name.value.trim())
      : await championshipService.create(name.value.trim())
    emit('saved', result)
  } catch (e) {
